...
# Local imports...
from mockingapi.services import get_todos, get_uncompleted_todos
from mockingapi.constants import SKIP_REAL


class TestTodos():
    @classmethod
    def setup_class(cls):
        cls.mock_get_patcher = patch('mockingapi.services.requests.get')
...
class TestUncompletedTodos():
    @classmethod
    def setup_class(cls):
        cls.mock_get_todos_patcher = patch('mockingapi.services.get_todos')
...
def test_integration_contract():
    ...
    with patch('mockingapi.services.requests.get') as mock_get:
