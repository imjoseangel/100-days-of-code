...
data = [['low', 'sunny', 'yes'], ['high', 'sunny', 'yes'],
        ['med', 'cloudy', 'yes'], ['low', 'raining', 'yes'],
        ['low', 'cloudy', 'no'], ['high', 'sunny', 'no'],
        ['high', 'raining', 'no'], ['med', 'cloudy', 'yes'],
        ['low', 'raining', 'yes'], ['low', 'raining', 'no'],
        ['med', 'sunny', 'no'], ['high', 'sunny', 'yes']]
...
# Scikit learn can't handle categorical data, so form numeric representations
# of the above data
# Categorical data support may be added in the future:
...
# Now that we have our decision tree, let's predict some outcomes from random
# data. This goes through each class and builds a random set of 5 data points
prediction_data = []
...
def print_table(tabledata, results):
    line = "day  " + format_array(list(classes.keys()) + ["went shopping?"])
    print("-" * len(line))
    print(line)
    print("-" * len(line))

    for day, row in enumerate(tabledata):
        print("{:<5}".format(day + 1) + format_array(row + [results[day]]))
    print("")


feature_names = (['supplies-' + x for x in classes["supplies"]] +
...
# Note that sklearn is unable to generate non-binary trees, so these are based
# on individual options in each class
